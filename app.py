...
VALID_CODES = ["888888", "666666", "123456", "sat1500"] # 您的预设授权码
...
def load_vocab():
    try:
        ...
        st.error("Error: Excel files not found! Please upload Core.xlsx, Advanced.xlsx, Sprint.xlsx")
...
# ==========================================
# 4. Core Engine (Macro Plan + Single Gen)
# ==========================================
else:
    ...
    # Not in a section yet: Show Start Button
    if not st.session_state.is_active_section:
        st.title("🎓 SAT Vocab Master Class")
        st.info("💡 **TIP for Mobile Users:** Tap the `>` arrow in the top-left corner to change difficulty mode and view the progress bar!")
    ...
    # Active Section
        ...
        # Generate Single Question Instantly
        if st.session_state.current_q_data is None:
            ...
            with st.spinner(f"🧠 Generating Question {current_idx + 1}... (approx. 3-5 seconds)"):
                prompt = f"""
                ...
                CRITICAL: Output ONLY a valid JSON. DO NOT use markdown code blocks like ```json. MUST BE 100% ENGLISH.
                
                Format:
                {{
                  "word": "{target_w}",
                  "domain": "{target_d}",
                  "passage": "<2-3 sentence highly academic English passage featuring logical contrast. Put '_____' where the word belongs.>",
